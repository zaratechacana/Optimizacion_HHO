...
num_hawks = 2  # Número de halcones
max_iterations = 6  # Número máximo de iteraciones
...
def budget_constraints(x1, x2, x3, x4, x5):
    # Restricciones específicas de cada medio
    budget_tv = 3800
    budget_dr = 2800
    budget_drr = 3500

    cost_tv = 160 * x1 + 300 * x2
    cost_dr = 40 * x3 + 100 * x4
    cost_drr = 40 * x3 + 10 * x5

    return (cost_tv <= budget_tv) and (cost_dr <= budget_dr) and (cost_drr <= budget_drr)
...
for t in range(max_iterations):
    for i in range(num_hawks):
        ...
        J = 2 * (1 - r5)  # Jump strength

        if np.abs(E) >= 1:  # Exploración
            hawks[i] = hawks[np.random.randint(num_hawks)] - r1 * np.abs(hawks[np.random.randint(num_hawks)] - 2 * r2 * hawks[i])
        else:  # Explotación
            if r1 >= 0.5 and np.abs(E) >= 0.5:  # Soft besiege
                hawks[i] = best_hawk - E * np.abs(J * best_hawk - hawks[i])
            elif r1 >= 0.5 and np.abs(E) < 0.5:  # Hard besiege
                hawks[i] = best_hawk - E * np.abs(best_hawk - hawks[i])
            elif r1 < 0.5 and np.abs(E) >= 0.5:  # Soft besiege with progressive rapid dives
                Y = best_hawk - E * np.abs(J * best_hawk - hawks[i])
                Z = Y + np.random.rand(len(LB)) * levy_flight(beta, len(LB))
                if quality_function(*[int(Y[j]) for j in range(5)]) > quality_function(*[int(hawks[i][j]) for j in range(5)]) and budget_constraints(*[int(Y[j]) for j in range(5)]):
                    hawks[i] = Y
                elif quality_function(*[int(Z[j]) for j in range(5)]) > quality_function(*[int(hawks[i][j]) for j in range(5)]) and budget_constraints(*[int(Z[j]) for j in range(5)]):
                    hawks[i] = Z
    ...
    for i in range(num_hawks):
        x1, x2, x3, x4, x5 = [int(hawks[i][j]) for j in range(5)]
        if budget_constraints(x1, x2, x3, x4, x5):  # Solo considerar soluciones que cumplan con las restricciones
            current_quality = quality_function(x1, x2, x3, x4, x5)
            if current_quality > best_quality:
                best_quality = current_quality
                best_hawk = hawks[i]
